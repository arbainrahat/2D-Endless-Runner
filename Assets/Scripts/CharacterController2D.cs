...
public class CharacterController2D : MonoBehaviour {
	...
	void Update()
	{
		...
		else if(canDoubleJump && Input.GetButtonDown("Jump")) // If canDoubleJump and jump button pressed, allow double jump
        {
			DoJump();

			//Disable double jump, so can only jump once again during jump
			canDoubleJump = false;
		}
	...
	//when collect coin than calulate score
    ...
    //DoJump() method for perform the function
    void DoJump()
    {
	...
	//Player bounce when hit head of enemy
	...
	public void Jump()
    {
		if (isGrounded) // If grounded AND jump button pressed, then allow the player to jump
		{
			DoJump();
		}
		else if (canDoubleJump) // If canDoubleJump and jump button pressed, allow double jump
		{
			DoJump();
	...
	public void HighScore()
    {
		if(PlayerPrefs.GetInt("score") <= score)
        {
			...
			Debug.Log("HighScore is : " + score);
		}
		highScoreTxt.text = PlayerPrefs.GetInt("score").ToString();
